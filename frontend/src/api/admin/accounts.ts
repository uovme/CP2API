...
export async function importCLIProxyAuth(payload: CLIProxyAuthImportRequest): Promise<CLIProxyAuthImportResult> {
  const { data } = await apiClient.post<CLIProxyAuthImportResult>('/admin/accounts/import/cliproxy-auth', payload)
  return data
}
