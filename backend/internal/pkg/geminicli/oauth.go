...
func BuiltinOAuthClientID() string {
	if clientID := strings.TrimSpace(GeminiCLIOAuthClientID); clientID != "" {
		return clientID
	}
	if clientID, ok := os.LookupEnv(GeminiCLIOAuthClientIDEnv); ok {
		return strings.TrimSpace(clientID)
	}
	return ""
}

func BuiltinOAuthClientSecret() string {
	if secret := strings.TrimSpace(GeminiCLIOAuthClientSecret); secret != "" {
		return secret
	}
	if secret, ok := os.LookupEnv(GeminiCLIOAuthClientSecretEnv); ok {
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// SECURITY: This repo does not embed built-in OAuth credentials; they must be provided via env.
	if effective.ClientID == "" && effective.ClientSecret == "" {
		clientID := BuiltinOAuthClientID()
		secret := BuiltinOAuthClientSecret()
		if clientID == "" {
	...
	isBuiltinClient := IsBuiltinOAuthClientID(effective.ClientID)
