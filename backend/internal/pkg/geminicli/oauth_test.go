...
const testBuiltinClientID = "test-gemini-cli-client-id"

func setTestBuiltinOAuthEnv(t *testing.T) {
	t.Helper()
	t.Setenv(GeminiCLIOAuthClientIDEnv, testBuiltinClientID)
...
func TestBuildAuthorizationURL_UsesBuiltinSecretFallback(t *testing.T) {
	...
	if !strings.Contains(authURL, "client_id="+testBuiltinClientID) {
...
func TestEffectiveOAuthConfig_GoogleOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantClientID: testBuiltinClientID,
...
func TestEffectiveOAuthConfig_MissingBuiltinEnv(t *testing.T) {
	...
	if err == nil {
		t.Fatal("不设置内置 OAuth 环境变量时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientIDEnv) {
		t.Fatalf("错误应提示 %s，实际: %v", GeminiCLIOAuthClientIDEnv, err)
	}
}
