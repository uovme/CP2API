...
**CLIProxyAPI + Sub2API 融合版 AI API 网关**
...
CP2API 是一个融合版 AI API 网关：以 Sub2API 的管理面板、API Key 分发、额度消耗记录和计费系统为基础，合入 CLIProxyAPI 风格的多账号 OAuth token 文件兼容、账号池调度和负载均衡体验。

用户通过平台生成的 API Key 调用上游 AI 服务，平台负责鉴权、用量记录、计费、负载均衡和请求转发。管理员可以导入 Sub2API 账号数据、Codex Session JSON，也可以直接导入 CLIProxyAPI `auths/*.json` token 文件。

## 核心功能

- **多账号管理** - 支持多种上游账号类型（OAuth、API Key）
- **CLIProxyAPI token 文件兼容** - 支持导入 `codex`、`claude`、`gemini`、`antigravity` auth JSON
- **两种 token 格式支持** - 同时支持 CLIProxyAPI auth JSON 和 Sub2API `credentials`/账号 JSON
...
CP2API 支持通过管理 API 导入 CLIProxyAPI 和 Sub2API 两类 token/账号数据：
...
- CLIProxyAPI auth JSON：`type=codex|claude|gemini|antigravity`
- Sub2API 账号/凭据 JSON：包含 `platform`、`type`、`credentials` 的对象
- JSON 数组、逐行 JSON、多文件内容数组

导入映射：

- `codex` -> `openai` OAuth 账号
- `claude` -> `anthropic` OAuth 账号
- `gemini` -> `gemini` OAuth 账号，默认 `oauth_type=code_assist`
- `antigravity` -> `antigravity` OAuth 账号

### 负载均衡和用量记录

CP2API 保留 Sub2API 的账号池调度、粘性会话、并发控制、速率限制、Token 级用量记录、账号/用户/API Key 统计和余额扣费能力；OpenAI/Codex 账号继续使用现有高级调度器和负载因子字段。
