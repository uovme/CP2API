...
**Merged AI API gateway for CLIProxyAPI and Sub2API workflows**
...
CP2API is a merged AI API gateway. It keeps Sub2API's dashboard, API key distribution, billing, quota tracking, and usage records, while adding CLIProxyAPI-style OAuth auth file compatibility for multi-account pools.

Users call upstream AI services through platform-generated API keys. CP2API handles authentication, usage recording, billing, load balancing, sticky sessions, and request forwarding.

## Features

- **Multi-Account Management** - Support multiple upstream account types (OAuth, API Key)
- **CLIProxyAPI Auth Import** - Import `codex`, `claude`, `gemini`, and `antigravity` auth JSON files
- **Dual Token Format Support** - Supports both CLIProxyAPI auth JSON and Sub2API account/credentials JSON
...
CP2API can import CLIProxyAPI and Sub2API account data through:

```http
POST /api/v1/admin/accounts/import/cliproxy-auth
```
...
{
  "contents": [
    "{\"type\":\"codex\",\"email\":\"user@example.com\",\"access_token\":\"...\",\"refresh_token\":\"...\",\"account_id\":\"acct_...\"}",
    "{\"type\":\"gemini\",\"email\":\"user@example.com\",\"project_id\":\"my-project\",\"token\":{\"access_token\":\"...\",\"refresh_token\":\"...\"}}"
  ],
  "group_ids": [1],
...
- CLIProxyAPI auth JSON with `type=codex|claude|gemini|antigravity`
- Sub2API account/credential JSON with `platform`, `type`, and `credentials`
- JSON arrays, JSON lines, and multiple file contents

Provider mapping:

- `codex` -> `openai` OAuth account
- `claude` -> `anthropic` OAuth account
- `gemini` -> `gemini` OAuth account with `oauth_type=code_assist`
- `antigravity` -> `antigravity` OAuth account

### Load Balancing and Usage Records

CP2API keeps Sub2API's account scheduler, sticky sessions, concurrency controls, rate limits, token-level usage logs, account/user/API-key statistics, and balance billing. OpenAI/Codex accounts continue to use the existing advanced scheduler and load factor fields.
